...
const BailCalculator = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (offenceDetails.isBailable) {
      setResult('The offence is bailable. Bail is granted as a matter of right.');
    } else {
      // Non-bailable offence checks
      if (sentenceHalfUndergone) {
        setResult('Bail may be granted if you have undergone half the sentence and the offence is not punishable with death or life imprisonment.');
      } else if (!investigationCompleted) {
        setResult('Bail may be granted if the investigation is not completed and the police report is not filed within 60 or 90 days.');
      } else if (!trialConcluded) {
        setResult('Bail may be granted if the trial before the Magistrate is not concluded within 60 days.');
      } else {
        setResult('Bail is not automatically granted and will be subject to judicial discretion based on the facts of the case.');
      }
    }
  };
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto bg-gray-50">
      <h1 className="text-3xl font-bold mb-6 text-center text-blue-800">Bail Calculator</h1>
      <p className="text-lg mb-6 text-center text-gray-700">
        Use this tool to check your eligibility for bail based on Indian law. Please provide details about the offence and your trial status.
      </p>

      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-lg">
        <div className="mb-4">
          <label htmlFor="offence" className="block text-gray-600 mb-2">Select Offence</label>
          <select
            id="offence"
            value={offence}
        ...
        {!offence || OFFENCE_TYPES.find(o => o.name === offence)?.isBailable ? null : (
          <>
            <p>Please tick appropriate options and submit</p><br />
            <div className="mb-4">
              <label htmlFor="sentenceHalfUndergone" className="inline-flex items-center">
                <input
                  type="checkbox"
                ...
                />
                <span className="ml-2 text-gray-600">Have you undergone half the sentence?</span>
              </label>
            </div>

            <div className="mb-4">
              <label htmlFor="investigationCompleted" className="inline-flex items-center">
                <input
                  type="checkbox"
                  ...
                  className="form-checkbox"
                />
                <span className="ml-2 text-gray-600">Is the investigation completed?</span>
              </label>
            </div>

            <div className="mb-4">
              <label htmlFor="trialConcluded" className="inline-flex items-center">
                <input
                  type="checkbox"
                  id="trialConcluded"
                  ...
                  className="form-checkbox"
                />
                <span className="ml-2 text-gray-600">Is the trial concluded?</span>
              </label>
            </div>
          </>
        )}
      ...
      {result && (
        <div className="mt-6 p-4 bg-white border border-gray-300 rounded-lg shadow-lg">
          <h2 className="text-xl font-semibold mb-2 text-blue-700">Eligibility Result</h2>
